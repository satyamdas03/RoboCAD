...
- Live end-to-end verified: `/decompose`, `/classify-domain`, `/generate?decompose=True`, `/designs/{id}/assembly-collision`, `/designs/{id}/assembly-poses`, `/designs/{id}/aero-report`, `/designs/{id}/thermal-report`, `/designs/{id}/cfd-mesh`, `/designs/{id}/electronics-report`, `/designs/{id}/idf-export`, `/robot-templates`, `/designs/{id}/robot-analysis`, `/designs/{id}/simulate` for humanoid/quadruped/manipulator-on-base, `/designs/{id}/world`, `/designs/{id}/world/randomize`, `/designs/{id}/world/replay`, `/designs/{id}/train-brain`, `/designs/{id}/brain-replay-attention`, and `/designs/{id}/brain` all work.
...
**Next work:**
1. **Phase 26 — HERMES cross-domain conversational supervisor:** build an LLM-driven orchestrator with tool use across design/simulation/training APIs, a status dashboard, explanation engine, and approval gates for expensive operations.
...
3. Complete remaining Phase 25 work: real MuJoCo closed-loop policy rollout harness (`WorldReplayEnv`), synthetic dataset generator (RGB/depth/segmentation), and design-feedback redesign loop.
4. Keep Phases 14A–25 under maintenance and the 414-test suite green.
...
> We are building **RoboCAD**, an AI-powered parametric CAD tool for robotics hardware. The repo is at `https://github.com/satyamdas03/RoboCAD`. **Phases 0–25 foundation are committed and pushed** (prompt → build123d → STL/STEP → Onshape → manufacturing report → component library → dark Kinetic Precision UI → feature tree → sketch constraints → assemblies → DFM/tolerances/FEA → model specialization → Claude 5 integration → GEDA Bridge → scene templates → LearningRobotics handshake → RoboCompiler pipeline → cross-domain input → domain-aware representation → automatic decomposition + part families → mechanical assembly synthesis → aero/thermal/propulsion geometry → electronics/mechatronics co-design → multi-physics verification engine → humanoid/full-robot system synthesis → hardened world-model simulation builder → attention-based robot brain training layer, **414 tests passing**). Latest commit `050c7d6` added `ComputeBudget`, `attention_regions`, `event_camera` sensor, actuator/sensor dropout randomization, per-body replay saliency, `compute_module` + `event_camera_mount` part families, and a NumPy-only `ai_cad/geda_bridge/brain/` package (`AttentionMLPPolicy`, `AbstractAttentionEnv`, `LinearWorldModel`, `AttentionBudget`, CEM trainer) plus backend `/train-brain` endpoints and `BrainTrainingPanel`. The immediate next phase is **Phase 26 — HERMES conversational supervisor**: an LLM-driven orchestrator with tool use, status dashboard, explanation engine, and approval gates. **Also queued:** robot arm cosmetic/mechanical refinement and remaining Phase 25 closed-loop work (real MuJoCo policy rollout, synthetic dataset generator, design-feedback loop). Strategic decision: ship PATH1 (GEDA Bridge) before PATH2 (voice-to-CAD-to-world-model). See `.claude/memory/robocad-path-analysis.md`, `.claude/memory/robocad-end-to-end-roadmap.md`, and `.claude/memory/phase25-attention-brain.md`. Say `:POINTBREAK` to force a full dossier sync.

---

*Last updated: 2026-09-01 (Phases 0–25 foundation complete; attention-based robot brain training layer shipped with compute-budget world-model extensions, NumPy-only CEM trainer, and `BrainTrainingPanel`; latest commit `050c7d6`; 414/414 tests passing across default, heavy/slow, and mujoco tiers; frontend build passes; HERMES conversational supervisor and robot arm cosmetic refinement are next)*
