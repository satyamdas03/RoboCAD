...
| 26–28 | End-to-end vision roadmap (Phase 26 HERMES conversational supervisor next) | `PLAN.md`, `.claude/memory/robocad-end-to-end-roadmap.md` |
...
**Current active phase:** Phase 25 foundation complete (414/414 tests passing across default, heavy/slow, and mujoco tiers; attention-based brain training layer shipped with `ComputeBudget`, `attention_regions`, `event_camera`, saliency replay, `ai_cad/geda_bridge/brain/` NumPy-only CEM trainer, `/train-brain` endpoints, and `BrainTrainingPanel`). Phase 26 — HERMES conversational supervisor — is next. Robot arm cosmetic/mechanical refinement (fillets, chamfers, joint bosses, tapered links, shaped jaws) is also queued.
