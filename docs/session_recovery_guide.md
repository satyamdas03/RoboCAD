...
23. `phase26-hermes-plan.md` — HERMES cross-domain conversational supervisor foundation: `ai_cad/hermes/` package (models, tool registry, approval gate, planner, session persistence, agent, explanation engine), FastAPI `/hermes/session*` endpoints, frontend `HermesPanel`; 454 tests passing (Phase 26 foundation complete; Phase 27 next).
...
| 26 | HERMES cross-domain conversational supervisor (foundation) | `ai_cad/hermes/`, `web/backend/main.py`, `web/frontend/src/components/HermesPanel.jsx`, `tests/test_hermes.py`, `tests/test_hermes_backend.py` |
...
**Current active phase:** Phase 26 foundation complete (454/454 tests passing across default, heavy/slow, and mujoco tiers; HERMES conversational supervisor shipped with `ai_cad/hermes/` package, `/hermes/session*` endpoints, and `HermesPanel`). Phase 27 — real-world feedback loop / sim-to-real — is next. Robot arm cosmetic/mechanical refinement (fillets, chamfers, joint bosses, tapered links, shaped jaws) remains queued. HERMES hardening (real LLM tool use, parameter validation, wire tools to live backends, design-feedback loop) is parallel work.
