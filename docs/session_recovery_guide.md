...
21. `phase24-world-simulation.md` — World-model simulation builder: `WorldDescription`, `WorldBuilder`, domain templates (pick-place, push, walker, drone hover, humanoid stand), domain randomization, MuJoCo MJCF + Isaac Sim JSON export + schema validation, body-name alias resolver, procedural terrain variants (stairs/ramp/uneven), rich replay capture (contacts/actuators/sensors), backend `/world` endpoints, frontend `WorldBuilderPanel`; 376/376 tests passing (Phase 24 complete).
22. `phase25-attention-brain.md` — Attention-based robot brain training layer: applied AI chip co-design ideas (attention, event-driven sensing, compute budgets, dynamic pruning) to world builder and NumPy-only `ai_cad/geda_bridge/brain/` package; `BrainTrainingPanel`, `/train-brain` endpoints, `compute_module` + `event_camera_mount` part families, saliency replay; 414 tests passing (Phase 25 foundation complete; Phase 26 next).
23. `phase26-hermes-plan.md` — HERMES cross-domain conversational supervisor, now complete end-to-end: real tool executors (`executor.py`), Pydantic parameter validation (`validation.py`), design-context builder (`context.py`), LLM caller (`llm.py`), backend callable wiring, design-feedback loop (`propose_redesign` → `regenerate_parameters`), session pruning, and enhanced `HermesPanel` with tool-result/redesign cards; 450/451 tests passing (1 expected failure, 5 benchmark/network tests deselected) (Phase 26 complete; Phase 27 next).
...
| 25 | Robot brain training loop (foundation) | `ai_cad/geda_bridge/brain/`, `ai_cad/geda_bridge/world_builder.py`, `web/backend/main.py`, `web/frontend/src/components/BrainTrainingPanel.jsx`, `tests/test_geda_bridge_brain.py` |
| 26 | HERMES cross-domain conversational supervisor | `ai_cad/hermes/executor.py`, `ai_cad/hermes/validation.py`, `ai_cad/hermes/context.py`, `ai_cad/hermes/llm.py`, `ai_cad/hermes/agent.py`, `ai_cad/hermes/session.py`, `web/backend/main.py`, `web/frontend/src/components/HermesPanel.jsx`, `tests/test_hermes*.py` |
| 27–28 | Real-world feedback loop + distribution/commercialization | `PLAN.md`, `.claude/memory/robocad-end-to-end-roadmap.md` |
...
**Current active phase:** Phase 26 complete end-to-end (450/451 tests passing across default, heavy/slow, and mujoco tiers, 1 expected failure, 5 benchmark/network tests deselected; HERMES conversational supervisor hardened with `ai_cad/hermes/executor.py`, `validation.py`, `context.py`, `llm.py`, `/hermes/session*` endpoints, and `HermesPanel`). Phase 27 — real-world feedback loop / sim-to-real — is next. Robot arm cosmetic/mechanical refinement (fillets, chamfers, joint bosses, tapered links, shaped jaws) remains queued.
